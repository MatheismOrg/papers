...
String theory is exponentially disfavored under every standard parsimony measure. It requires at minimum 3,000 bits to reproduce observed physics: 1,661 bits to index one vacuum from a landscape of 10^500 candidates, plus flux integers and moduli values. With moduli stabilization and supersymmetry breaking, the total reaches 4,700 bits. The Standard Model's 26 scalar parameters require at most 390 bits at measurement precision (26 parameters at 15 bits each), and potentially far fewer if compressible generating rules exist, as they do for mathematical constants like pi and other infinite-digit irrationals that can be generated from 12 bits. Under algorithmic probability, compressible constants are exponentially favored. Even with zero SM compression, the gap exceeds 2,600 bits, a prior odds ratio exceeding 2^2,600. Astronomical. We state the comparison as a proposition about explicit code lengths, not exact Kolmogorov complexity; the conclusion is code-relative and conditional, a prior penalty, not a refutation. Under algorithmic probability, elegance is short description. A selector requiring 1,661 bits is not elegant. String theory landscape-selection has so far failed its own aesthetic and parsimony criteria on the same grounds.

**Keywords:** string theory, Standard Model, string landscape, naturalness, parsimony, vacuum selection, Kolmogorov complexity, algorithmic probability, minimum description length, Bayesian model selection, description-length bounds
...
String theory routes through a landscape of at least 10^500 vacuum solutions, each requiring a compactification geometry, flux integers, and stabilized moduli. Indexing one vacuum costs log2(10^500), approximately 1,661 bits, absent a compressing selection rule. None has been found. Total string-theoretic specification: 3,000 bits minimum (vacuum index plus flux), 4,700 bits with moduli stabilization and SUSY breaking.

The asymmetry is structural. Scalar-parameter descriptions have constants that are potentially compressible under algorithmic probability. Landscape-selection descriptions have a selector cost bounded below by enumeration unless a generating rule replaces it. Even comparing SM uncompressed (390 bits) against string theory at its best (3,000 bits), the gap exceeds 2,600 bits. The prior probability ratio is at least 2^2,600. String theory, given the need for landscape-selection, is not slightly disfavored. It is exponentially, astronomically, disfavored. The productive alternative is a causally invariant hypergraph rewrite rule simple enough to be SDP-favored, whose stable topological features reproduce the Standard Model's particle spectrum: three generations, three gauge groups, the observed mass hierarchy. Current hypergraph efforts operate at 10^−93 to 10^−100 meters: a gap too vast to bridge by brute-force simulation. The more promising direction starts from the known sub-atomic values and searches downward until hypergraph rules produce them, using perhaps the most variational rules such as those involving the golden ratio. The search focuses on the simplest causally invariant rules. Rather than selecting from 10^500 vacua, the task is finding the minimal rule whose graph knots and twists ARE the particles. SDP predicts this rule exists and is short.
...
Total description length: L \= log₂(N) \+ B. If a compressing rule exists that replaces the explicit enumeration, the cost collapses to the length of that rule. The burden of exhibiting such a rule falls on the proponent.
...
These are explicit description lengths under the coding scheme above. The true K of a string-theoretic realization may be strictly smaller if shorter generating rules exist for some components. K\_U(M\_ST) is at most these explicit lengths plus a machine-dependent constant.

## **6\. The description-length penalty**

**Proposition.** Let L\_SM and L\_ST be explicit description lengths of a scalar-parameter theory and a landscape-selection theory under the same prefix code. Under the universal prior, P(ST)/P(SM) ≤ 2^(-(L\_ST \- L\_SM) \+ c), where c is a small machine-dependent constant. The proof is immediate: P(M) is proportional to 2^(-K(M)), K is bounded above by explicit code length, and the ratio follows by substitution.

**Corollary.** For L\_ST \- L\_SM approximately 2,940 bits (Section 5), the prior odds ratio is at most 2^(-2,940), absent a compressing selector rule.
