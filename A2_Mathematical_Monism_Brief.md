...
Some consequences follow from the foundation before any physics. The hard problem of consciousness is circular reasoning, all interactive dualisms collapse to mathematical monism, and the supernatural and libertarian free will resolve as square circles. The physics follows.

## 2. Occam's Razor Is a Theorem

Scientists prefer simpler theories. This preference has been treated as methodological taste, aesthetic bias, or pragmatic convenience. Hossenfelder (2018) asked why physics should prefer simplicity at all, and found no satisfactory answer. SDP provides the answer: lower-K structures have exponentially higher measure. The criterion is not beauty but brevity.

A theory with generating complexity K₁ describes structures with measure proportional to 2^(−K₁). A competitor with complexity K₂ > K₁ describes structures with measure proportional to 2^(−K₂). The ratio is 2^(K₂ − K₁), exponentially favoring the simpler theory. Occam's Razor is not a guide to inference. It is a fact about the distribution of existence - the Simplicity Dominance Principle (SDP).
...
Algorithmic probability adds precision: among observer-permitting structures, simpler ones are exponentially more probable. This predicts that the fundamental constants are not arbitrary but among the simplest values compatible with observers. The constants themselves form a complexity hierarchy: the golden ratio φ (~3 bits), √2 (~4), e (~8), π (~10). Nature deploys the simplest constant matching each structural requirement. The open question is which hypergraph rewrite rule reproduces the Standard Model's particles as stable topological features; current efforts operate at 10^−100 meters, far below observational access, so the productive direction starts from known sub-atomic values and searches downward. It also predicts that outstanding puzzles like dark matter will have simple solutions (lower-K explanations will be confirmed over higher-K alternatives), and that the initial conditions we observe are low in complexity because observers are more likely to find themselves computing from simple starting points. The same bias applies to temporal location: earlier observer-moments have shorter specification prefixes, but later moments have more total observers. The distribution peaks where these pressures meet. Algorithmic probability also applies to competing physical theories: string theory's total specification complexity (approximately 3,000 to 4,700 bits) dwarfs any plausible generating rule for the Standard Model's 19 to 26 scalar parameters, rendering string theory exponentially disfavored. Under SDP, elegance is short description. String theory landscape-selection has so far failed its own aesthetic and parsimony criteria on the same grounds. Under mathematical monism, all string vacua exist, but low complexity of a theory itself is necessary and not sufficient: the theory must also reproduce observed physics, and no string vacuum has been shown to do so.
...
The K-decomposition clarifies the comparison. Total specification complexity is K(L) + K(I) + K(O), where K(L) is laws, K(I) is initial conditions, and K(O) is the specification of which particular measurement outcomes occurred. Under collapse interpretations, K(O) grows without bound: each measurement selects one eigenvalue, accumulating an algorithmically incompressible string. Under both Many-Worlds and superdeterminism, K(O) = 0. No random selection occurs. Feynman called the double slit the only mystery of quantum mechanics. It is only a supernatural anti-causal random mystery if you add collapse. Von Neumann's 1932 "proof" that hidden variables are impossible assumed what it aimed to prove. Hermann (1935) identified the flaw; Bell (1966) confirmed it.

The fine-tuning objection inadvertently argues against collapse, not against superdeterminism. The Born rule, whether probabilities proportional to squared amplitudes follow from SDP or require independent derivation, remains an open problem. Many-Worlds retains a parsimony advantage with current models (the Schrödinger equation alone, no additions), but the competition is mathematical: whichever generating rule has lower total K wins.

Einstein's own ontology (block universe from SR/GR, determinism, hidden variables from EPR) jointly entails superdeterminism, a connection he never made because Bell formalized the key assumption nine years after his death. In a block universe, "initial conditions" is misleading: there is no temporal beginning at which conditions are arranged. The correlations are geometric features of a timeless structure. The conspiracy objection presupposes a temporal setup that the block universe excludes. The tools to formalize this connection, Kolmogorov complexity and Solomonoff induction, postdated Einstein; SDP completes the path his ontology implied .
...
SDP makes a meta-prediction: future puzzles in fundamental physics will have simple solutions. Complexity is the exception, not the rule. If the pattern holds, algorithmic probability is confirmed. If a future discovery requires irreducibly high-K explanation, algorithmic probability is challenged. SDP is falsifiable through its consequences.
