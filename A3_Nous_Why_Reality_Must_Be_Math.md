...
The jet bundle reconstruction theorem provides a further line of support. For an analytic metric, the infinite set of derivatives at any single spacetime point reconstructs the entire geometry everywhere via Taylor's theorem (Bernstein, 2026o). Every point holographically encodes the whole, not as a physical postulate from black hole thermodynamics, but as a mathematical consequence of calculus applied to an analytic manifold. This is precisely what mathematical monism predicts: if reality is mathematical structure, the full structure should be deducible from its local properties, because the local properties are the structure.
...
Bernstein, G. A. (2026d). Why these simple laws? Deriving physics from mathematical necessity. https://independent.academia.edu/TheMatheist
