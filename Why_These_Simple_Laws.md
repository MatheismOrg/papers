# **Why These Simple Laws? Mathematical Necessity, Algorithmic Probability, and the Measure Problem**
...
This is the structuralist conception defended by Ladyman and Ross (2007): entities are "nodes" in a relational network, with no identity independent of their position in the structure. The claim to be established is that *all* coherent existence takes this form.
...
Algorithmic probability is defined over countably many programs, each generating countably many outputs. Structures requiring selection from uncountable continua (rather than continuous dynamics over computable values) are unreachable by this measure. They exist mathematically but have measure zero. Fundamental physics must therefore be discrete; continuous descriptions (general relativity, classical field theory) are emergent approximations. Theories with continuous dynamics but finite specification (such as string theory) remain consistent with IPP; the constraint is finite specifiability, which discrete approaches satisfy manifestly.
...
**Combinatorial justification:** Among all programs of length ≤ n bits, shorter programs vastly outnumber longer ones. There are 2^k programs of length k, so programs of length k are exponentially more numerous than programs of length k+1. If we imagine a universal Turing machine randomly generating programs, shorter programs are exponentially more likely to be generated. This is Solomonoff's (1964) foundation for inductive inference; IPP extends it from epistemology to ontology. Schmidhuber (1997, 2000, 2002\) developed this into algorithmic theories of everything; the present argument provides the a priori grounding for why the ensemble exists and why algorithmic probability governs it.
...
**Prediction 6:** Fundamental physics is discrete. Continuous descriptions (general relativity, classical field theory) are emergent approximations of underlying discrete structure. Quantum gravity approaches assuming discrete foundations (loop quantum gravity, causal sets, Wolfram's hypergraphs) align with IPP; approaches treating continua as fundamental do not.

Current physics is consistent with all six predictions. The Standard Model, despite its complexity, derives from remarkably compact symmetry principles. General relativity follows from a one-line action. Quantum mechanics admits elegant formulations. Cross-domain mathematical transfer is pervasive.

Wolfram's Physics Project exemplifies the theoretically predicted approach: the hypothesis that our universe emerges from extremely simple hypergraph rewriting rules (potentially specifiable in just a few bits) is not merely consistent with IPP but required by it. Discrete foundations are measure-theoretically necessary, not methodologically optional. If such a project succeeds in deriving Standard Model physics from minimal rules, IPP receives strong corroboration. However, verification faces computational barriers: the hypergraph primitives may operate at scales around 10^−93 meters, nearly 10^58 times smaller than the Planck length, making direct simulation intractable. Mathematical derivation rather than brute-force computation may be required to bridge the gap between simple rules and emergent physics.
...
CPP suggests we should expect to find ourselves early in our structure's observer-window. We do.
